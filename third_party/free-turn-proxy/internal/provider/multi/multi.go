...
func (m *Provider) GetCredentials(ctx context.Context, streamID int) (provider.Credentials, error) {
	...
	credentials, err := p.GetCredentials(ctx, innerID)
	if err != nil && m.n > 1 && errors.Is(err, provider.ErrFatalNoStreams) {
		// A fatal result belongs to this VK call, not to the combined transport.
		// Removing the sentinel keeps the relay alive so another independent call
		// can finish its CAPTCHA and start serving streams.
		return credentials, fmt.Errorf("provider[%d] unavailable: %s", idx+1, err)
	}
